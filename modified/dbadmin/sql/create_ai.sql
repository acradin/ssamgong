...
CREATE TABLE chatbot_description_t (
    cd_idx INT AUTO_INCREMENT PRIMARY KEY,
    ct_idx INT NOT NULL,
    cd_description TEXT NOT NULL,
    cd_wdate DATETIME NOT NULL,
    UNIQUE KEY `uk_ct_idx` (`ct_idx`),
    FOREIGN KEY (ct_idx) REFERENCES category_t(ct_idx) ON DELETE CASCADE
...
(8, 1, '참고 자료', 'PDF 파일을 업로드해주세요', 'file',
    NULL, 'Y', 8, NOW()),
(8, 1, '기타 요구사항', '추가적인 요구사항을 입력해주세요', 'text',
    NULL, 'N', 9, NOW()),
...
(9, 1, '참고 자료', 'PDF 파일을 업로드해주세요', 'file',
    NULL, 'Y', 8, NOW());
(9, 1, '기타 요구사항', '추가적인 요구사항을 입력해주세요', 'text',
    NULL, 'N', 9, NOW()),
