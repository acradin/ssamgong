...
try {
    ...
    if (isset($_GET['main_category']) && !empty($_GET['main_category'])) {
        $where .= ($where ? ' AND ' : '') . 'ph.main_ct_idx = ?';
    ...
    // 포인트 내역 조회
    $query = "SELECT ph.*, m.mt_id, m.mt_nickname, m.mt_name, m.mt_hp, m.mt_email,
              parent.ct_name as main_category_name,
              child.ct_name as sub_category_name
              FROM point_history_t ph
              LEFT JOIN member_t m ON ph.mt_idx = m.mt_idx
              LEFT JOIN category_t child ON ph.ct_idx = child.ct_idx
              LEFT JOIN category_t parent ON ph.main_ct_idx = parent.ct_idx";
    if ($where) {
        $query .= " WHERE " . $where;
    }
    $query .= " ORDER BY ph.ph_idx DESC LIMIT ?, ?";
    $params[] = $offset;
    $params[] = $limit;
    ...
    echo '</select>
            ...
            <div class="form-group">
                ...
                <button type="button" onclick="location.href=\'point_history.php\'">초기화</button>
            </div>
          </form>';
    ...
    if (!empty($histories)) {
        ...
        foreach ($histories as $history) {
            ...
            // 포인트 유형에 따른 스타일과 텍스트 설정
            $pointTypeText = '';
            $pointTypeClass = '';
            switch($history['point_type']) {
                case 'free':
                    $pointTypeText = '무료 사용';
                    $pointTypeClass = 'point-free';
                    break;
                case 'use':
                    $pointTypeText = '유료 사용';
                    $pointTypeClass = 'point-use';
                    break;
                default:
                    $pointTypeText = $history['point_type'];
                    $pointTypeClass = 'point-default';
            }

            // 카테고리 정보 표시
            $categoryText = '-';
            if (!empty($history['main_category_name'])) {
                $categoryText = htmlspecialchars($history['main_category_name']);
                if (!empty($history['sub_category_name'])) {
                    $categoryText .= ' > ' . htmlspecialchars($history['sub_category_name']);
                }
            }
            ...
            echo '<tr>
                    ...
                    <td class="point-amount ' . ($history['point_amount'] < 0 ? 'minus' : 'zero') . '">' 
                        . ($history['point_amount'] < 0 ? '-' : '') 
                        . number_format(abs($history['point_amount'])) . '</td>
                    <td class="' . $pointTypeClass . '">' . htmlspecialchars($pointTypeText) . '</td>
                    <td>' . $categoryText . '</td>
